...
class VideoVisualizer:
    ...
    def draw_panoptic_seg_predictions(
        self, frame, panoptic_seg, segments_info, area_threshold=None, alpha=0.2
    ):
        frame_visualizer = Visualizer(frame, self.metadata)
    ...
    def _assign_colors(self, instances):
        ...
        colors_for_allowed_categories = {
            0:  [1.000, 0.000, 0.000], #person
            1:  [1.000, 0.000, 0.000], #bycicle
            2:  [0.000, 0.000, 1.000], #car
            3:  [0.000, 0.000, 1.000], #motorcycle
            9:  [1.000, 1.000, 0.000], #traffic light
            11: [1.000, 1.000, 0.000], #stop sign
        }

        # Assign random color to newly-detected instances:
        for inst in instances:
            if inst.color is None:
                #@Todo farben alle einer instance zu einer pro category
                if inst.label in colors_for_allowed_categories:
                    inst.color = colors_for_allowed_categories.get(inst.label)
                if inst.color is None:
