...
class DetectionPainter:
    ...
    def annotation(self, ax, ann, *, color=None, text=None, subtext=None):
        ...
        if(subtext == 1):
            # Load image
            fn = get_sample_data('e:/input/pedestrian1.png', asfileobj=False)
            arr_img = plt.imread(fn, format='png')
            imagebox = OffsetImage(arr_img, zoom=zoom) 
            imagebox.image.axes = ax
        elif (subtext == 2):
            # Load image
            fn = get_sample_data('e:/input/pedestrian2.png', asfileobj=False)
            arr_img = plt.imread(fn, format='png')
            imagebox = OffsetImage(arr_img, zoom=zoom) 
            imagebox.image.axes = ax
        elif (subtext == 3):
            # Load image
            fn = get_sample_data('e:/input/pedestrian3.png', asfileobj=False)
            arr_img = plt.imread(fn, format='png')
            imagebox = OffsetImage(arr_img, zoom=zoom) 
            imagebox.image.axes = ax
        else:
            # Load default error image
            fn = get_sample_data('e:/input/pedestrianError.png', asfileobj=False)
            arr_img = plt.imread(fn, format='png')
            imagebox = OffsetImage(arr_img, zoom=zoom) 
            imagebox.image.axes = ax

        # draw image
        ab = AnnotationBbox(imagebox, [x + w/2, y - h/2], frameon=False)
        ax.add_artist(ab)
'''
        # draw box
        ax.add_patch(
